...
def test_prepare_nnunet_dataset_rejects_geometry_mismatch(tmp_path: Path) -> None:
    dataset_dir = tmp_path / "dataset"
    write_nifti(dataset_dir / "case_a" / "image.nii.gz", np.zeros((4, 4, 3), dtype=np.uint8))
    write_nifti(dataset_dir / "case_a" / "mask.nii.gz", np.zeros((4, 4, 2), dtype=np.uint8))
    write_nifti(dataset_dir / "case_b" / "image.nii.gz", np.zeros((4, 4, 3), dtype=np.uint8))
    write_nifti(dataset_dir / "case_b" / "mask.nii.gz", np.zeros((4, 4, 3), dtype=np.uint8))
    write_nifti(dataset_dir / "case_c" / "image.nii.gz", np.zeros((4, 4, 3), dtype=np.uint8))
    write_nifti(dataset_dir / "case_c" / "mask.nii.gz", np.zeros((4, 4, 3), dtype=np.uint8))
    ...
    with pytest.raises(ValueError, match="Image/mask shape mismatch for case_a"):
        prepare_nnunet_dataset(config_path)
