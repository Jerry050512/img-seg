...
#cover(
  ...
  date: [2026 年 7 月],
  status: [COURSE REPORT · RESULTS AS OF 2026-07-10],
)
...
截至 2026 年 7 月 10 日，本机已完成 nnU-Net v2 2D fold 0 的 200 epoch 实验。最佳 checkpoint 在 2 个独立测试 case 上取得宏平均 Dice 0.9490、前景 IoU 0.9048、二类 mIoU 0.9192、Accuracy 0.9587、Precision 0.9581 和 Recall 0.9405。模型约含 46.32M 参数；按单张 512×512 输入、乘法和加法分别计一次 FLOP，卷积及转置卷积计算量约为 119.27 GFLOPs。对尺寸恰为 512×512×291 的 `S_3` 使用 RTX 4060 Laptop GPU 进行整例推理，端到端耗时 32.079 s，折合约 110.2 ms/slice 或 9.07 FPS。MONAI SegResNet 与 EfficientNet-B0 U-Net 的实验结果尚未汇总，报告使用醒目的占位符保留对应位置，不将未完成结果伪装为真实测量值。

#callout(
  "结果口径",
  [报告中的 nnU-Net 数值均来自运行 `nnunet_v2_2d_fold0_200epochs_20260710_114102`。训练日志的 pseudo Dice、整例验证指标和独立测试指标含义不同，本文分别呈现；模型横向结论在另外两组结果补齐前仅视为阶段性结论。],
)

#v(0.45cm)
#text(font: "Microsoft YaHei", size: 9pt, fill: muted, weight: "bold")[关键词：] NIfTI；二值语义分割；nnU-Net v2；SegResNet；EfficientNet-B0；case-level split；WebUI
...
#table(
  ...
  table-text[#placeholder[待补充]], table-text[#raw("@NH-5")], table-text[负责人 B], table-text[EfficientNet-B0 编码器的 2D U-Net 训练、推理与结果分析], table-text[30%（暂定）],
  table-text[#placeholder[待补充]], table-text[#raw("@flypigff")], table-text[负责人 C], table-text[MONAI SegResNet 的 3D patch 训练、推理与结果分析], table-text[30%（暂定）],
)

#callout(
  "提交前检查",
  [请补齐三位成员的真实姓名、学号，确认工作量比例，并将另外两组模型的 checkpoint、测试指标、收敛曲线与定性图替换到本文占位位置。],
  tone: "gold",
)
...
#table(
  ...
  table-text[性能指标], table-text[第 4 章给出公式，第 7 章给出 nnU-Net 实测值与统一对比表], table-text[#tag[部分完成]],
  table-text[复杂度与效率], table-text[46.32M Params、119.27 GFLOPs；RTX 4060 上 9.07 FPS/110.2 ms], table-text[#tag[已测 nnU-Net]],
  table-text[收敛曲线与雷达图], table-text[由 200 epoch 日志和独立测试混淆矩阵生成], table-text[#tag[已完成]],
  table-text[3 类定性图], table-text[高吻合、弱边界漏分、复杂边界误分], table-text[#tag[已完成]],
  table-text[三模型对比], table-text[结构与配置已齐；另外两模型结果待负责人补齐], table-text[#tag(tone: red)[待补结果]],
...
#table(
  ...
  table-text[Validation], table-text[973×973×312；1319×914×245], table-text[2], table-text[32.90%、44.37%], table-text[checkpoint 选择],
  table-text[Test], table-text[973×973×298；512×512×291], table-text[2], table-text[40.97%、42.76%], table-text[独立评估],
)

固定随机种子为 42，并按 case 划分为 train=9、validation=2、test=2。nnU-Net 的 `imagesTr/labelsTr` 含 train 与 validation 共 11 例，`splits_final.json` 再将 fold 0 固定为 9/2；2 个测试 case 仅用于最终推理和评价。该设计保证同一体数据的全部切片只能属于一个集合。
...
另外两条模型线的配置目标如下：EfficientNet-B0 U-Net 对 z 轴切片缩放至 512×512，保留全部前景切片并仅保留 25% 空 mask；MONAI SegResNet 使用 96×96×64 的 3D ROI、滑窗推理和 0.5 overlap。它们仍须在训练代码中确认增强实现与配置一致。
...
项目现有 `metrics.json` 中字段 `iou` 指前景 IoU。为严格满足课程的 mIoU 要求，本文利用保留的 TP/FP/FN/TN 另算背景 IoU，并取二者平均。最佳 nnU-Net 的宏平均前景 IoU 为 0.9048、背景 IoU 为 0.9337，因此二类 mIoU 为 0.9192。
...
当前配置：输入/输出通道均为 1，初始 filters=16，ROI=96×96×64，batch size=1，200 epoch，AdamW，learning rate=2e-4，AMP 开启，Dice+BCE loss；推理采用 overlap=0.5 的 sliding window。以下内容尚待负责人提交真实运行证据：

#callout(
  "MONAI SegResNet 结果占位",
  [#placeholder[最佳 checkpoint 路径、训练时长、验证/测试指标、Params、FLOPs、FPS、收敛曲线、3 类定性图与简短结论]],
  tone: "red",
)
...
当前配置：100 epoch，batch size=8，AdamW，learning rate=3e-4，AMP，Dice+BCE，阈值 0.5；保留全部含前景切片和 25% 空切片，体推理后逐层重组 NIfTI。以下内容尚待负责人提交真实运行证据：

#callout(
  "EfficientNet-B0 U-Net 结果占位",
  [#placeholder[最佳 checkpoint 路径、训练时长、验证/测试指标、Params、FLOPs、FPS、收敛曲线、3 类定性图与简短结论]],
  tone: "red",
)
...
本机主要硬件为 13th Gen Intel Core i7-13700H、约 16 GB 内存和 NVIDIA GeForce RTX 4060 Laptop GPU。训练日志记录 PyTorch 2.5.1+cu121、CUDA 设备 `cuda:0`。Windows 图形驱动环境下可用显存和连续内存分配并不总是一致，大尺寸 case 在模型迁移阶段仍可能触发分配失败。
...
WebUI 不直接调用某个模型内部层，而通过公共 inference 模块完成输入收集、模型/权重选择、推理、输出二值化、结果评价和预览。当前界面支持：
...
#table(
  ...
  fill: (x, y) => if y == 0 { navy } else if calc.odd(y) { paper-blue } else { white },
  table-head[模型], table-head[mIoU], table-head[Dice], table-head[Acc.], table-head[Prec.], table-head[Recall], table-head[Params/M], table-head[FLOPs/G], table-head[FPS / ms],
  table-text[nnU-Net v2 2D], table-text[*0.9192*], table-text[*0.9490*], table-text[*0.9587*], table-text[*0.9581*], table-text[*0.9405*], table-text[46.32], table-text[119.27], table-text[9.07 / 110.2],
  table-text[MONAI SegResNet], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]],
  table-text[EfficientNet-B0 U-Net], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]], table-text[#placeholder[—]],
)

#callout(
  "横向结论暂缓",
  [表中只有 nnU-Net 为实测值。另两模型未完成前，不能声称 nnU-Net“最好”、EfficientNet“最快”或 SegResNet“最稳”。最终对比必须使用同一 test split、同一阈值、同一 512×512 复杂度口径和同一硬件计时协议。],
  tone: "gold",
)
...
#figure(
  ...
  caption: [nnU-Net 最佳 checkpoint 的独立测试指标雷达图。mIoU 为前景/背景两类 IoU 均值，其他指标为 case 宏平均。],
) <fig:radar>

雷达图显示五项指标均高于 0.91，Precision 高于 Recall，说明总体更偏向保守预测，漏分风险高于误分风险。这与 `2_25_XY` 的 FN 可视化相符。另两模型完成后应在同一坐标范围内叠加曲线；如同时考虑速度和参数量，建议另绘 Dice-FPS 散点图，并用点面积表示 Params。
...
#table(
  ...
  table-text[Params], table-text[46.324558 M], table-text[实际 2D PlainConvUNet，2 类输出头],
  table-text[FLOPs], table-text[119.2680 G], table-text[1×1×512×512；卷积/转置卷积；MAC=2 FLOPs],
  table-text[`S_3` CUDA], table-text[32.079 s/case], table-text[512×512×291，端到端，TTA off],
  table-text[`S_3` 摊销速度], table-text[9.07 FPS / 110.2 ms], table-text[整例耗时除以 291 slices],
  table-text[`2_25_XY` CPU], table-text[821.835 s/case], table-text[973×973×298，显存分配失败后 CPU fallback],
  table-text[两例混合均值], table-text[426.957 s/case], table-text[混合 CPU/CUDA，仅用于复现实验，不宜横向排名],
)

速度数值包含预处理、网络、重采样和保存。119.27 GFLOPs 仅计网络主干卷积，两者不是互相推导关系。最终三模型效率对比应使用固定 512×512 输入，预热后同步 CUDA，至少重复 100 次并报告均值、P50 和 P95；当前结果保留了更接近实际部署的端到端口径。
...
小样本是当前最主要的统计风险。两个测试 case 无法覆盖全部形态变化，0.9490 的均值也不能给出稳定置信区间。其次，大尺寸 case 的 CPU fallback 使平均速度无法代表统一硬件性能。再次，Accuracy 受背景比例影响，不应取代 Dice/mIoU。最后，标注流程元数据不完整会影响标签可信度和误差归因。
...
同时，Precision、Recall 和定性误差图必须与 Dice 一起理解。`2_25_XY` 的 Recall 较低，提示模型对弱边界采用保守策略；单看 0.9156 Dice 很难发现具体失败模式。三维上下文可能改善连续性，但 3D 模型也会带来显存、训练时间和过拟合风险。因此应根据数据规模和部署目标选择模型，而不是默认复杂模型一定更优。
...
本项目已经完成数据规范化、case-level 划分、nnU-Net 训练/验证/测试、模型复杂度统计、三类定性分析和批量推理 WebUI。7 月 10 日最佳 nnU-Net checkpoint 在当前独立测试集上达到 Dice 0.9490、二类 mIoU 0.9192，证明自配置 2D U-Net 是可靠的阶段性强基线；但验证 Dice 仅 0.7737，且验证/测试样本各只有 2 例，因此不能据此做强泛化结论。

报告尚未完成的关键项是 MONAI SegResNet 与 EfficientNet-B0 U-Net 的真实实验结果，以及组员姓名/学号确认。最终提交前应补齐两模型指标和图表，在统一硬件协议下重测 512×512 延迟，并优先增加 5-fold case-level 交叉验证。完成这些工作后，才能对三种模型的精度、复杂度与部署价值给出可信的横向结论。
...
#table(
  ...
  table-text[MONAI SegResNet 完整实验], table-text[#tag(tone: red)[待补]], table-text[#placeholder[负责人补充]],
  table-text[EfficientNet-B0 U-Net 完整实验], table-text[#tag(tone: red)[待补]], table-text[#placeholder[负责人补充]],
