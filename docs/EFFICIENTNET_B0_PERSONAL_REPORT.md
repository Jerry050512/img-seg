...
`docs/EXPERIMENTS.md` 还记录了 nnU-Net v2 2D 200 轮实验；MONAI SegResNet 尚无完成指标。现有记录的测试对象不同，只能作背景信息，不能作公平排名：
...
| nnU-Net v2 2D 200 epochs | `H1-20Layer`，单例记录 | 未记录 | 0.9751 | 0.9514 | 0.9779 | 0.9723 | 58.8 s |
| MONAI SegResNet | 无完成记录 | 未记录 | 未记录 | 未记录 | 未记录 | 未记录 | 未记录 |

nnU-Net 的数值更高，但其测试病例和样本数与 EfficientNet-B0 不同，不能据此得出模型优劣结论。最终三模型横向对比必须让三者使用同一冻结测试集、相同二值化规则和相同病例宏平均口径，并补充峰值显存后再进行。
